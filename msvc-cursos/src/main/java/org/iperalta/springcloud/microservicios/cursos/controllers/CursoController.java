package org.iperalta.springcloud.microservicios.cursos.controllers;
...
import org.iperalta.springcloud.microservicios.cursos.services.CursoService;
...
public class CursoController {
    ...
    public ResponseEntity<?> eliminar(@PathVariable Long id) {
        Optional<Curso> o = service.porId(id); // buscamos para validar que exista y eliminar
        if (o.isPresent()) {
            service.eliminar(o.get().getId());
            return ResponseEntity.noContent().build();
        }
        return ResponseEntity.notFound().build(); // sino existe el id
    }

    // localhost:8002/asignar-usuario/1
    @PutMapping("/asignar-usuario/{cursoId}")
    public ResponseEntity<?> asignarUsuario(@RequestBody Usuario usuario, @PathVariable Long cursoId) {
        Optional<Usuario> o;
        try {
            o = service.asignarUsuario(usuario, cursoId); // Aqui el usuario si tiene "id"
    ...
    public ResponseEntity<?> crearUsuario(@RequestBody Usuario usuario, @PathVariable Long cursoId) {
        Optional<Usuario> o;
        try {
            o = service.crearUsuario(usuario, cursoId); //Aqui el usuario que llega no tiene "id" porque se va crear
        } catch (FeignException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
